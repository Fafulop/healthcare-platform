          ...
          return (
            <div key={purchase.id} className={`bg-white rounded-lg shadow p-4 ${selectedIds.has(purchase.id) ? 'ring-2 ring-blue-400' : ''}`}>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                <InlineStatusSelect currentStatus={purchase.status} statuses={statusOptions} onStatusChange={async (s) => { await onPurchaseStatusChange(purchase.id, purchase.status, s); }} disabled={updatingId === purchase.id} />
