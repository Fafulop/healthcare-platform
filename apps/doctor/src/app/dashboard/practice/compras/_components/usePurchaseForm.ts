...
import type { Supplier, Product, PurchaseItem } from './purchase-types';
...
export function usePurchaseForm() {
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await authFetch(`${API_URL}/api/practice-management/proveedores?status=active`);
  ...
  const updateItemQuantity = (tempId: string, quantity: number) => {
    setItems(prev => prev.map(item => {
      ...
      const subtotal = base - base * item.discountRate;
  ...
  return {
    // Data
    suppliers, setSuppliers, products, loadingSuppliers, loadingProducts,
