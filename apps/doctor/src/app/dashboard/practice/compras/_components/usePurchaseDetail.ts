...
export interface Purchase {
  ...
  supplier: Supplier;
  quotation: PurchaseQuotation | null;
  items: PurchaseItem[];
}
...
export function usePurchaseDetail() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'bold');
      doc.text(formatDateLong(purchase.purchaseDate), 14, y);
      doc.text(purchase.deliveryDate ? formatDateLong(purchase.deliveryDate) : 'No especificada', 100, y);
      y += 10;
  ...
  return { purchase, loading, error, exportingPDF, handleExportPDF };
}
