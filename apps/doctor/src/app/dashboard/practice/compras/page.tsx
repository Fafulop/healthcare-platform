...
import { useComprasPage } from './_components/useComprasPage';
...
export default function ComprasPage() {
  ...
  const {
    ...
    selectedIds, filteredPurchases, totalPurchases, totalPaid, totalPending,
    updatingId, editingAmountPaidId, editingAmountPaidValue, setEditingAmountPaidValue, updatingAmountPaid,
    fetchPurchases, fetchDoctorProfile,
  ...
  useEffect(() => {
    fetchPurchases(statusFilter, paymentFilter, page);
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm sm:text-base">Gestiona tus compras a proveedores</p>
            </div>
            <div className="flex gap-2">
              <Link href="/dashboard/practice/proveedores" className="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition-colors font-semibold">
                <Truck className="w-5 h-5" /><span className="hidden sm:inline">Proveedores</span>
              </Link>
              <Link href="/dashboard/practice/compras/new" className="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors font-semibold">
                <Plus className="w-5 h-5" /><span className="hidden sm:inline">Nueva Compra</span>
              </Link>
            </div>
          </div>
        </div>
