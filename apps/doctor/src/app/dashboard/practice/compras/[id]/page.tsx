...
import { usePurchaseDetail, statusConfig, paymentStatusConfig } from "../_components/usePurchaseDetail";
...
export default function ViewPurchasePage() {
  ...
  const { purchase, loading, error, exportingPDF, handleExportPDF } = usePurchaseDetail();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/dashboard/practice/compras" className="text-blue-600 hover:text-blue-700">
            Volver a Compras
          </Link>
        </div>
      </div>
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">{formatDateLong(purchase.purchaseDate)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Fecha de entrega</p>
              <p className="text-lg font-semibold text-gray-900">
                {purchase.deliveryDate ? formatDateLong(purchase.deliveryDate) : 'No especificada'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-8">
            ...
            <div className="space-y-2">
              ...
              {purchase.supplier.contactName && <p className="text-gray-700">Contacto: {purchase.supplier.contactName}</p>}
              {purchase.supplier.email && <p className="text-gray-700">Email: {purchase.supplier.email}</p>}
              {purchase.supplier.phone && <p className="text-gray-700">Teléfono: {purchase.supplier.phone}</p>}
              {purchase.supplier.rfc && <p className="text-gray-700">RFC: {purchase.supplier.rfc}</p>}
