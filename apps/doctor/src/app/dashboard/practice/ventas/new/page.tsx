...
export default function NewVentaPage() {
  ...
  }, [clients, selectedClientId, selectedPatient, saleDate, deliveryDate, paymentStatus, amountPaid, notes, termsAndConditions, items]);

  // Chat IA callbacks
  const handleChatFieldUpdates = useCallback((updates: Record<string, any>) => {
    if (updates.clientName) {
      // Fuzzy-match client or patient (same logic as handleVoiceConfirm)
    ...
    if (updates.saleDate) setSaleDate(updates.saleDate);
    ...
    if (updates.amountPaid !== undefined) setAmountPaid(updates.amountPaid);
