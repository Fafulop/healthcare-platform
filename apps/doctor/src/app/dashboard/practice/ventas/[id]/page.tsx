...
export default function ViewSalePage() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'bold');
      doc.text(pdfFormatDate(sale.saleDate), 14, y);
      doc.text(sale.deliveryDate ? pdfFormatDate(sale.deliveryDate) : 'No especificada', 100, y);
      y += 10;
      ...
      // Adjust y to be after client box
      y = Math.max(y + 6, y + 2);
      ...
      if (amountPaid > 0) {
        ...
        if (balance > 0) doc.setTextColor(220, 38, 38); // red-600
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">
                  {sale.deliveryDate ? formatDate(sale.deliveryDate) : 'No especificada'}
                </p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              ...
              <div className="space-y-2">
                ...
                {sale.client.contactName && (
                  <p className="text-gray-700">Contacto: {sale.client.contactName}</p>
                )}
                {sale.client.email && (
                  <p className="text-gray-700">Email: {sale.client.email}</p>
                )}
                {sale.client.phone && (
                  <p className="text-gray-700">Teléfono: {sale.client.phone}</p>
                )}
                {sale.client.rfc && (
                  <p className="text-gray-700">RFC: {sale.client.rfc}</p>
                )}
                {sale.client.street && (
