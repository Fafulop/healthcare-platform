...
type ModalTab = "block" | "unblock";

function formatBlockLabel(bt: BlockedTime): string {
  const time = bt.startTime === "00:00" && bt.endTime === "23:30"
    ? "Todo el día"
...
export function BlockTimeModal({ isOpen, onClose, blockTime, unblockTimes, blockedTimes, onSuccess }: Props) {
  ...
  const toggleUnblockId = (id: string) => {
  ...
  const handleUnblockSelected = async () => {
    ...
    const count = selectedUnblockIds.size;
    if (!await practiceConfirm(`¿Desbloquear ${count} horario(s) seleccionado(s)?`)) return;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-lg max-w-lg w-full my-4 sm:my-8">
        ...
        {/* Tab toggle */}
        <div className="px-4 sm:px-6 pt-3 pb-0 flex gap-1 bg-gray-50 border-b">
          <button
            type="button"
            onClick={() => setTab("block")}
            className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${
              tab === "block"
                ? "bg-white text-orange-700 border border-b-white border-gray-200 -mb-px"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${
              tab === "unblock"
                ? "bg-white text-green-700 border border-b-white border-gray-200 -mb-px"
                : "text-gray-500 hover:text-gray-700"
            }`}
          >
            ...
            <>
              {blockedTimes.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Los bloqueos que crees aparecerán aquí</p>
                </div>
              ) : (
                <>
                  {/* Select all / count */}
                  <div className="flex items-center justify-between">
                    ...
                    {selectedUnblockIds.size > 0 && (
                      <span className="text-xs text-green-700 bg-green-50 px-2 py-0.5 rounded-full">
                        {selectedUnblockIds.size} seleccionado(s)
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                    {sortedBlockedDates.map((dateKey) => (
                      <div key={dateKey}>
                        <p className="text-xs font-semibold text-gray-500 mb-1">{dateKey}</p>
                        {blockedByDate.get(dateKey)!.map((bt) => {
                          const isChecked = selectedUnblockIds.has(bt.id);
                          return (
                            <button
                              key={bt.id}
                              type="button"
                              onClick={() => toggleUnblockId(bt.id)}
                              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg border mb-1 text-left transition-all ${
                                isChecked
                  ...
                  <div className="flex gap-2 justify-end pt-3 border-t">
                    ...
                    <button
                      ...
                      disabled={selectedUnblockIds.size === 0 || isUnblocking}
                      className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white font-medium rounded-lg transition-colors text-sm flex items-center gap-1.5"
                    >
                      {isUnblocking ? <Loader2 className="w-4 h-4 animate-spin" /> : <Unlock className="w-4 h-4" />}
                      Desbloquear {selectedUnblockIds.size > 0 ? `(${selectedUnblockIds.size})` : ""}
                    </button>
                  </div>
                </>
              )}
