...
import { camposParaElAgente, dictParaRender, formatoDe, leerPdfBase, etiquetasPorClave } from '@/lib/informe-medico/formatos';
...
export async function POST(
  ...
  try {
    ...
    // Mismo veto que en el chat: este endpoint también le ofrece campos a un
    // modelo, así que no puede tener un catálogo más permisivo.
    const campos = camposParaElAgente(
      camposDictables(geo, pagina, { ...contexto, ...etiquetasPorClave(formato) }),
      formato
    );
    if (campos.length === 0) {
