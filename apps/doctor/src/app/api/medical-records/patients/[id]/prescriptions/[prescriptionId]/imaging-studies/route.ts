...
// GET /api/medical-records/patients/:id/prescriptions/:prescriptionId/imaging-studies
export async function GET(
  ...
  try {
    ...
    const studies = await prisma.prescriptionImagingStudy.findMany({
    ...
    return handleApiError(error, 'GET imaging-studies');
  }
}

// POST /api/medical-records/patients/:id/prescriptions/:prescriptionId/imaging-studies
export async function POST(
  ...
  try {
    ...
    const maxOrder = await prisma.prescriptionImagingStudy.findFirst({
      where: { prescriptionId },
      orderBy: { order: 'desc' },
    });

    const study = await prisma.prescriptionImagingStudy.create({
      data: {
        prescriptionId,
    ...
    await logAudit({
      ...
      action: 'add_prescription_imaging_study',
    ...
    return handleApiError(error, 'POST imaging-studies');
  }
}
