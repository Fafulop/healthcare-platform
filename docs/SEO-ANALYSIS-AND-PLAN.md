...
### Phase 1: On-Page SEO Optimization (HIGH PRIORITY — 2-3 days)

These changes directly affect how Google understands what each profile page is about.

| # | Task | What to Change | Impact |
|---|------|----------------|--------|
| 1.1 | **Add city to H2 subtitle** | `HeroSection.tsx`: Change H2 from `{specialty}` to `{specialty} en {city}` | HIGH — Strongest on-page signal linking specialty to location |
| 1.2 | **Add intro paragraph in hero** | `HeroSection.tsx`: Add 2-3 sentence `<p>` below H2 using `short_bio` with natural specialty+city keywords | HIGH — Gives Google keyword-rich prose above the fold |
...
These fix the root causes so future profile edits don't reintroduce SEO problems.
...
<h2>Oftalmologo en Guadalajara</h2>                              -- FIXED: city added
<p>Oftalmologo especialista en Guadalajara con 15+ anos...</p>   -- NEW: keyword-rich intro
<h2>Servicios de Oftalmologia</h2>                                -- FIXED: specialty in heading
<h2>Condiciones y Procedimientos Oftalmologicos</h2>              -- FIXED: specialty in heading
[service descriptions visible on mobile via line-clamp]           -- FIXED: mobile content visible
<h2>Consultorio en Guadalajara</h2>                               -- FIXED: city in heading
```
