...
| Códigos ya expuestos | los `confirmationCode` leídos antes del cierre seguían sirviendo para cancelar | ✅ decisión del usuario: **regenerar** los de citas activas futuras (`scripts/security/rotate-confirmation-codes.cjs`, 37 citas / 4 doctores en el ensayo, corrido DESPUÉS del cierre). La bitácora no registra QUIÉN cancela: no se puede saber si hubo abuso; la línea base de cancelaciones (13–33/mes) no muestra picos |
