...
| E7 | "¿A qué hora me desocupo hoy?" (cita con bloque extendido) | `extendedBlockMinutes` no venía en el mapping → una cita con extensión se reportaba libre desde su fin nominal (en prod hay extensiones de 60–705 min) | `BOOKING_SELECT` incluye `extendedBlockMinutes`; la cita trae **`ocupadoHasta`** = `max(fin, inicio + ext)` — ⚠️ la extensión cuenta **desde el INICIO de la cita** (misma fórmula que `availability-calculator`), NO desde el fin: la 1ª versión sumaba desde el fin y reportó 15:32 donde la UI (correcta) decía 14:47 — detectado por el doctor en pruebas en vivo. Solo se emite si supera el fin nominal; solo PENDING/CONFIRMED |
