...
| [`09-ANALISIS-recortar-superficie-del-agente.md`](09-ANALISIS-recortar-superficie-del-agente.md) | **¿Conviene darle MENOS tools al agente para que se equivoque menos?** La decisión (2026-07-30): **no por ahora**, con el reencuadre que la sostiene — la línea no es lectura vs escritura sino **fontanería** (de ahí salen los IDs, no se puede quitar) vs **lectura-respuesta** — y los disparadores que harían reconsiderarla | vivo |
| [`10-ANALISIS-especializar-agente-por-area.md`](10-ANALISIS-especializar-agente-por-area.md) | **¿Conviene PARTIR el agente y especializarlo por área (citas, facturas)?** La decisión (2026-08-01): **no todavía** — cae en Nivel 3 de la escalera y ninguna señal ha disparado. Trae la **trampa de medición** (los esquemas de tools van diferidos: medir un scope sumando sus tools da un ahorro FALSO), la evidencia de que Haiku no tiene fallos estables, y las tres palancas más baratas sin ejercer | vivo |
| [`11-ANALISIS-contexto-de-pantalla.md`](11-ANALISIS-contexto-de-pantalla.md) | **¿Cómo hacemos que el panel sepa qué está viendo el doctor?** Disección de la arquitectura navegador↔LLM↔humano de *Gemini in Chrome* (el Page Content Agent, los IDs de nodo, el filtrado en el extractor, los checkpoints por consecuencia) y **qué pieza copiamos con nuestro modelo y nuestras tools**. Retoma el pendiente que `01-PLAN` §5 difirió. Trae la restricción de caché (§6), **un QUINTO eje de fuga por recortar** (§7) y el riesgo inverso: Gemini ya lee nuestras pantallas en México (§10) | vivo |
| [`12-PLAN-contexto-de-pantalla.md`](12-PLAN-contexto-de-pantalla.md) | **El CÓMO del 11.** 🌱 Nada construido — pausado por prioridad, listo para retomarse. Trae las **4 verificaciones contra el código que hay que hacer ANTES** (§2), fase 0 (la ruta) y fase 1 (la entidad) con archivos, los **evals de fuga que hoy no existen y son bloqueantes** (§6), cómo se mide si sirvió (§7), y **por qué "tools por pantalla" es la versión cara de la idea** (§8.1) | plan |
| [`01-PLAN-panel-copilot-persistente.md`](01-PLAN-panel-copilot-persistente.md) | El panel acoplado que sobrevive la navegación. **SHIPPED** — el doc conserva las desviaciones del plan (§7), los follow-ups abiertos F1–F4 (§8) y las lecciones de Tailwind (§9). ⚠️ Su §5 dejó **fuera de alcance el contexto de pantalla**: ese hilo se retoma en [`11-ANALISIS`](11-ANALISIS-contexto-de-pantalla.md) | snapshot |
...
- **Pendientes conocidos**: PR 4 (voz + retiro del ChatWidget v1 + limpieza `/v1` `/v2`) ·
  F3 de facturas · la migración de los 8 `*-chat` heredados. (Los 2 bugs de conducta del agente
  —#23 card fantasma y #24 over-claim— se corrigieron 2026-07-23; A4 prefijo re-medido 2026-07-23.)
