...
| `llm-assistant/chat` | C | gpt-4o-mini | **el ChatWidget flotante** (RAG sobre los docs), montado en `app/dashboard/layout.tsx` ⇒ visible en TODO el dashboard | ⚠️ faltaba en este inventario (agregado 2026-08-27). Su modelo NO vive con los demás: `lib/llm-assistant/constants.ts` (`LLM_MODEL`). El pipeline RAG muere con él en PR 4 (04-PLAN §2). ⚠️ **2026-09-22: el `ChatWidget` ya NO se monta** (se retiró `WIDGET_AYUDA_VISIBLE`); su lugar es de `ayuda/chat`, abajo. El endpoint sigue vivo sin UI que lo llame |
| `ayuda/chat` | C | **gpt-4o-mini** (por `AYUDA_MODELO`) | **el widget de Ayuda NUEVO** — contesta cómo se usa la app desde `lib/ayuda/manual-del-doctor.md` (manual entero en el prompt, sin RAG). Toma el lugar del `ChatWidget` de arriba en la pila flotante | agregado 2026-09-22. Cero tools, cero datos del doctor. **En TODOS los planes** (sin `feature: 'ia'` en el route map, a propósito) con tope diario `AYUDA_TOPE_DIARIO` (60). El servidor tira citas y enlaces que no existen. Docs: `../../AYUDA WIDGET/` |
