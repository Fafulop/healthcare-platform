...
> 5 módulos · suite 65 casos · prefijo **27,151 MEDIDO exacto** con `count_tokens` (2026-07-23;
> corrige el "~24.7k" estimado — era +10% menor. Ninguna señal §5.3 disparada, nivel 0 se
> mantiene, pero con menos headroom del que se creía).**
...
- **Uso esporádico (el doctor real): cada pregunta con >5 min de silencio RE-PAGA el write del
  ...
  > 
  > 📏 **Re-medición del mismo día (2026-07-23, tarde — baseline de costo + `count_tokens`):**
  > la predicción "~25-30k con 5-6 módulos" de este párrafo **acertó**: el prefijo MEDIDO exacto
  > es **27,151** (el "~24.7k" del A4 era un piso inferido, −10%). Dos correcciones a los números
  > de arriba: (1) la pregunta fría real cuesta **41,331 budget ($0.083)**, no ~33k — el 82% es
  > escribir el prefijo; (2) el cap ya no es diario 500k sino **semanal 2M**, así que la cuenta
  > vigente es **~48 preguntas frías/semana (~7/día)**. Todo sigue siendo dr-prueba.
  > Ver [`../OPTIMIZACION COSTOS/`](../OPTIMIZACION%20COSTOS/README.md).
...
- Podar secciones de prompt por módulo (~2-3k tokens es el presupuesto; si un módulo pide más,
  ...
  > 📏 **Medido 2026-07-23 — 3 de 5 módulos exceden ese presupuesto:** facturas **8,706** (~3×),
  > agenda **7,255** (~2.4×), flujo 3,032 (apenas); fiscal 1,590 y expediente 1,598 ✅. Por la
  > regla de esta misma línea, facturas y agenda merecen mirada de ARQUITECTURA (¿qué veredictos
  > siguen resolviéndose en el prompt?), no solo tijera de prosa. Blancos tool por tool en
  > [`../OPTIMIZACION COSTOS/01-PLAN`](../OPTIMIZACION%20COSTOS/01-PLAN-experimentos.md) §2b.
