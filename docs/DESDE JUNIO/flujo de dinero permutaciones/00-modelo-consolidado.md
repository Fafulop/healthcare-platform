...
- `metodoPago` (PUE/PPD) **sí** se captura: vive en `SatCfdiDetail.metodoPago` y
  `LedgerFacturaXml.metodoPago`.
- Los complementos de pago (tipo P) **sí** se rastrean: en `SatPago` (liga `pagoUuid → facturaUuid`
  con `montoPagado`, `saldoInsoluto`, `numParcialidad`).
- **PERO** el auto-registro (`apps/api/src/lib/sat-auto-register.ts:388`) hace, sin condición:
  ```ts
  const paymentStatus = isReceived ? 'PENDING' : 'PAID';
  ```
  → **todo CFDI emitido se marca `PAID`**, ignore `metodoPago`. Una factura PPD que emitiste a
  Elanco (sin dinero aún) queda registrada como ingreso **ya pagado**. Esto contradice la verdad.
- Además, el auto-registro solo procesa `efecto ∈ {I, E}` y **salta `P` (pagos)**. El complemento
  que **prueba** que el dinero del PPD llegó se descarga a `SatPago` pero **no** alimenta el
  `paymentStatus` del ledger principal.

> **Este es el gap #1 respecto al objetivo "saber dónde está el dinero":** hoy el sistema confunde
> *evidencia fiscal* (existe la factura) con *realidad bancaria* (el dinero se movió) para los PPD.
> El `PPD ledger redesign` (en `../../PERMUTATIONS/ppd-ledger-redesign.md`) es un **plan** para
> exponer esto, aún no integrado a `LedgerEntry`.
...
**D. Factura PPD a una empresa (caso Elanco)** — el gap §3
1. Emites CFDI PPD → hoy cae como `sat_emitido`, **PAID** (❌ debería ser PENDING — sin dinero aún)
2. Semanas después Elanco paga → llega un **complemento (tipo P)** → se guarda en `SatPago` pero
   **no** se refleja en el entry
3. Llega el depósito bancario → Motor 3 *puede* adjuntar 🏦
Hoy: 🧾✓ pero el estado de pago es incorrecto desde el paso 1, y el complemento es invisible en el
ledger principal.
...
1. **PUE/PPD no respetado** *(gap #1)* — CFDIs emitidos marcados PAID sin condición; `metodoPago`
   ignorado; complementos `SatPago` no alimentan el `paymentStatus` del entry.
